  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="w-full max-w-md max-h-[92vh] bg-ui-background rounded-t-2xl overflow-hidden flex flex-col relative"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
          ...
          >
            <div className="w-12 h-1.5 rounded-full bg-ui-line mx-auto mt-2.5 mb-1 shrink-0 pointer-events-none" />
                    ...
                    <div key={item.id} className="flex items-center gap-3 bg-ui-surface rounded-2xl p-3">
                      ...
                      <div className="flex flex-col items-end gap-2 shrink-0">
                        <button
                          ...
                          className="text-ui-ink-muted hoverable-btn rounded-full p-1"
                        >
                          <Trash2Icon size={14} />
                        </button>
                        <div className="flex items-center gap-2 bg-ui-background rounded-full px-2.5 py-1">
                          <button
                          ...
                          <span className="font-mono text-sm text-ui-ink w-4 text-center">{item.qty}</span>
                          <button
                            onClick={() => updateQty(item.id, item.qty + 1)}
                            className="text-ui-ink hoverable-btn rounded-full"
                          >
